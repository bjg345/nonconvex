...
if(n <= 250) {
	maxprec.fit = fit.water(grid.train, A.train, y=vals.train, nu = .5, method='cov.select', n.neighbors=k)
    }else{
		maxprec.fit = readRDS(file.path('cs_fit', paste0('fit_', n, '_', stdev, '_', id, '.rds')))
	} 
maxprec.pred = sapply(1:nrow(grid.test), function(i) pred_neighbor(new.loc = grid.test[i,], A=A.train, A.vec = NULL, 
